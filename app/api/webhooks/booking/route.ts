...
import { upsertContact, createBookedDeal } from '@/lib/hubspot';
...
// Sync to HubSpot: update the contact and create a deal at the booked
// stage. Independent try/catch so a HubSpot outage never blocks the SMS
// confirmation or Slack notification above.
let hubspotDealId: string | null = null;
    try {
        const contactId = await upsertContact({ first_name, last_name, email, phone });
        hubspotDealId = await createBookedDeal(contactId, `${name || email || phone} - booked call`);
        await db.from('inbound_touch_log').insert({
            lead_id: leadId, day: 0, channel: 'hubspot', template_key: 'deal_booked',
            status: 'sent', external_id: hubspotDealId, content: 'HubSpot contact updated + deal created',
        });
    } catch (e: any) {
        await db.from('inbound_touch_log').insert({
            lead_id: leadId, day: 0, channel: 'hubspot', template_key: 'deal_booked',
...
return NextResponse.json({ lead_id: leadId, booking_id: booking?.id, hubspot_deal_id: hubspotDealId });
}
